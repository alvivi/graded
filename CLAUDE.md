...
Knowledge base priority: user externals (spec file) > project sibling modules (cache) > dependency spec files (`build/packages/<dep>/<dep_spec_file>`) > path dependencies (spec file or fallback to source inference) > versioned catalog (`priv/catalog/`, `{package}@{version}.graded`, highest version ≤ installed) > Unknown. Record constructors (`Ok`, `Some`, custom types) are pure and skipped during extraction.
