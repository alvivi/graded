...
**assay** is an effect checker for Gleam that uses sidecar `.assay` annotation files to verify properties the type system can't reach. Written in Gleam, targeting the Erlang/BEAM runtime.

Annotations live in `priv/assay/` alongside Gleam source. The primary consumer is LLM agents, not humans.
...
gleam run -m assay check [dir]   # check effect annotations
gleam run -m assay infer [dir]   # infer and write effect annotations
gleam run -m assay format [dir]  # format .assay files
```
...
| `annotation` | Parse/format `.assay` sidecar files |
...
| `assay` | CLI entry point: find files, orchestrate, print results |

## .assay Annotation Syntax
...
- `effects` — inferred cache, auto-generated by `assay infer`
