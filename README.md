...
- **Second-order (nested) effect variables are supported, with one inference caveat.** Effects are represented as a small lambda-calculus-with-union (`EffectTerm`); `EffectSet` is its ground normal form. A *higher-kinded* effect variable — an operator `Eff → Eff` rather than a flat `Eff` — arises when a parameter's own type takes a function (`action: fn(fn() -> Nil) -> a`). A call to such an operator parameter, `action(cb)`, infers an effect-operator *application* `[action(Stdout)]`, and at a call site an operator-typed argument is lifted to an operator and the application **beta-reduces** to the concrete effect. Transitive first-order propagation (`outer(f) → middle(f) → inner(f)` ⟹ `[f]`, `apply2(f, g) : [f, g]`) still works as before. See [docs/second-order-effects.md](./docs/second-order-effects.md) for the design and the property suite. Operator arguments are lifted from a **named function reference** (abstracting over its callback parameter, resolved via the knowledge base) *and* from an **inline closure** (analysing its body, abstracting over its first parameter). **Caveat:** an operator argument that is an *opaque local* — a value graded can't trace to a function or closure (e.g. `let h = pick_handler()`) — has no callback parameter to abstract over, so its application stays stuck and collapses to `[Unknown]`; annotate it explicitly if needed. (A named-function operator argument also needs that function already in the knowledge base — inferred earlier in the topological pass, or in the catalog.)
