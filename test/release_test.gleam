...
// girard catalog
//
// The `girard@2.0.0` entry: a consumer resolves girard's effects from the
// catalog instead of `[Unknown]`, and the polymorphic `annotate` bound leaves
// an `[Unknown]` residue rather than a closed set that drops whatever the
// caller's own resolver does.

fn girard_fixture(name: String, module: String) -> String {
  write_fixture("/tmp/graded_release_girard_" <> name, [
    #("gleam.toml", "name = \"app\"\nversion = \"1.0.0\"\n"),
    #(
      "manifest.toml",
      "packages = [
  { name = \"girard\", version = \"2.0.0\" },
  { name = \"gleam_stdlib\", version = \"0.70.0\" },
]
",
    ),
    #("annotate.gleam", module),
  ])
}

pub fn girard_call_resolves_from_catalog_test() {
  let directory =
    girard_fixture(
      "catalog",
      "import girard
...
pub fn make_resolver() {
  girard.disk_resolver()
}

pub fn describe(t) {
  girard.type_to_string(t)
}
",
    )

  let assert Ok(Nil) = graded.run_infer(directory)
  let cache = directory <> "/build/.graded/annotate.graded"

  // Both calls collapse to [Unknown] with no catalog entry.
  read_inferred_effect(cache, "make_resolver")
  |> should.equal(labels(["FileSystem"]))

  read_inferred_effect(cache, "describe")
  |> should.equal(pure())

  cleanup(directory)
}

// `annotate` is polymorphic in the resolver its options carry, so a call whose
// options graded can't trace reports `[Unknown]`. The reverted entry declared a
// bare `[FileSystem]` here, closing the set over an effect graded never saw.
pub fn girard_annotate_does_not_understate_caller_options_test() {
  let directory =
    girard_fixture(
      "opaque",
      "import girard

pub fn run(source, options) {
  girard.annotate(source, options)
}
",
    )
  ...
  read_inferred_effect(directory <> "/build/.graded/annotate.graded", "run")
  |> should.equal(labels(["Unknown"]))

  cleanup(directory)
}

// A caller-supplied resolver that prints keeps its [Stdout] through the
// `annotate` bound. [FileSystem] comes from `default_options`, [Unknown] is the
// residue of the unresolved bound.
pub fn girard_annotate_keeps_caller_resolver_effects_test() {
  let directory =
    girard_fixture(
      "resolver",
      "import gleam/io
...
pub fn run(source) {
  let resolver = fn(path) {
    io.println(path)
    Error(Nil)
  }
  girard.annotate(source, girard.with_resolver(girard.default_options(), resolver))
}
",
    )
  ...
  read_inferred_effect(directory <> "/build/.graded/annotate.graded", "run")
  |> should.equal(labels(["FileSystem", "Stdout", "Unknown"]))
