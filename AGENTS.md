...
Each Gleam package has a single spec file (default `<package_name>.graded`) at the project root holding hand-written `check`/`assume` annotations alongside the inferred public-API effects — none for the modules `internal_modules` in `gleam.toml` names (by default `<name>/internal` and `<name>/internal/*`). A per-module cache at `build/.graded/` holds the inferred effects for *all* functions (public and private) and is regenerated by `graded infer`. Both locations are configurable via `[tools.graded]` in `gleam.toml`.
