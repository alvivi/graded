...
variables** — effect *operators* of kind `Eff → Eff`, the step beyond the flat
`Eff` sets of first-order polymorphism.

[Goal](#goal) motivates them, [The core reframe](#the-core-reframe) gives the
representation (a small `EffectTerm` lambda calculus), and
[Resolution coverage](#resolution-coverage) summarises every program form graded
resolves through. The remaining sections cover the data model, reduction
semantics, surface syntax, and the property suite.
...
graded lifts a function value into an operator (or a flat effect) from many
program forms, and resolves the result wherever it is used.

**Operators are n-ary, by currying.** An operator parameter whose type takes
several functions (`fn(fn() -> _, fn() -> _) -> _`) threads *all* its callbacks as
a curried application `((action e1) e2)`, and an operator argument is lifted to a
...
- **Named function references** — cross-module via the knowledge base; same-module
  via on-demand transitive analysis (siblings aren't in the KB during their own
  module's inference pass).
- **Inline closures** and **let-bound closures** (`let h = fn(cb) { … }`).
- **`case`/`if` branches over function-like options** — each branch is lifted, and
  the operators are **joined** by descending their `TAbs` spines in lockstep:
  `(f ⊔ g)(cb) = f(cb) ⊔ g(cb)`.
...
- A **block** resolves to its tail expression.
- A record **field wired to a closure** resolves from the closure body — including
  an **operator-typed field** whose closure calls its callback (`λnext. [next]`,
  applied at the field call).
- A record **field wired to a parameter** resolves through a **factory** — a
  function whose tail constructs the record from its parameters; `let v =
  make(io.println)` then binds the result's fields like a direct construction.
- `check` **auto-infers** project modules missing from the spec.
- A returned operator may be **polymorphic in the producer's parameters**: a
  producer that returns one of its operator parameters (`fn wrap(base) { base }`),
  or **wraps** it in a closure (a decorator,
  `fn traced(action) { fn(cb) { log(); action(cb) } }`), binds the parameter to the
  producer call's argument. A returned closure is lazy, so it is excluded from the
  producer's own direct call-effect (counted only when applied), keeping decorators
  precise.
...
- a function value reached through *arbitrary computation* (would need
  whole-program control-flow analysis);
- a record receiver from an untraceable source (no provenance to a factory or
  construction);
- a `use`-tailed *returned operator*.
