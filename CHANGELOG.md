...
- `gleam.toml` now states a compiler requirement, `gleam >= 1.15.4`: the oldest
  Gleam on which graded builds, passes its suite, and on which the type
  inference's compiler differential evidence — regenerated by that compiler —
  and the coverage probe are both clean. Versions below it were not verified
  and are not claimed.
- `graded coverage` states the versions in play beside the ones graded was
  verified on, with a line naming any that is not one of them and any girard
  the analyzed project pins other than the one running.
