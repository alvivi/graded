...
- A dependency's spec with a line graded cannot read now uses every other line
  ...
  rest.
- A path dependency whose spec file is there but cannot be read — a directory at
  the spec path, bytes that are not UTF-8, denied permissions — is now inferred
  from its source, with a warning naming the cause, where it used to answer
  nothing at all and print nothing. A `check` over a call into it can newly pass
  or newly fail.
...
- `gleam.toml` now states a compiler requirement, `gleam >= 1.15.4`. Versions
  below it were not verified and are not claimed.
...
- A call through a local name that shadows an imported module is now charged as
  the module call the compiler emits, where the label is not on every variant of
  the receiver's type or sits at a differing position or type in one. Such a
  body — `gleam/io.println` called through a parameter named `io` — could be
  reported pure before; a receiver narrowed to one variant is unaffected, and a
  `check` line over one of the rest can start failing.
