...
- **A module-level `external effects <module>` declaration now applies to a path dependency's inferred module.** graded source-infers a path dependency that has no spec file of its own; that inference previously shadowed a consumer's module-level external, so a path-dep module declared pure with `external effects dep : []` still resolved to the inferred `[Unknown]`. The declaration now governs and the module resolves to the declared set. Hex dependencies were already unaffected, since graded never source-infers them. A per-function `external effects mod.fn`, or an authoritative dependency spec or catalog effect, still takes precedence.
