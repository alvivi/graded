...
- **Lustre 5 catalog.** A `lustre@5.0.0.graded` catalog file covers the v5 surface: the `application`, `simple`, `component`, and `element` constructors now resolve to `[]` (pure data) instead of `[Unknown]`, alongside the `named`, `dispatch`, `shutdown`, `supervised`, and `is_browser` value constructors, the effectful runtime functions (`start`, `register`, `send`, …), and the element/attribute/event submodules. Projects on Lustre 5.x select this file; Lustre 4.x projects keep `lustre@4.0.0.graded`.
- **`graded check` warns on spec lines that match nothing.** A `check` line whose function name matches no function in any project module — usually a missing module qualifier (`check view : []` instead of `check myapp.view : []`) — never runs and passes silently; it's now flagged. A `type` line is flagged when it can't resolve any field: unqualified, qualified at a module that's neither a project module nor an installed/path dependency (a typo), or naming a field of a project type that isn't function-typed. Whether a field is callable is resolved by following type-alias chains across project and dependency modules — a module-local alias (`type Handler = fn(...)`), an alias imported from another module (`callback: handlers.Handler`), and an alias that delegates to an imported one (`type Local = handlers.Handler`) all count, and a non-function dependency type (`value: types.Record`) is correctly flagged while a dependency-owned function alias is not. A field on a dependency-owned type, or one whose type graded can't resolve, is left alone rather than flagged. All warnings are reported against the spec file.
- **Function-typed record fields resolve polymorphically instead of `[Unknown]`.** A `fn`-typed field called on a receiver graded can't trace to a construction site — an opaque parameter — now becomes a *field-effect variable* named for its `receiver.field` access path instead of collapsing to `[Unknown]`. The variable discharges against a `check f(r.run: [Stdout]) : [..]` field bound or a `type myapp.Runner.run : [Stdout]` line, and surfaces as a polymorphic bound when the function is inferred (`effects f(r.run: [r.run]) : [r.run]`); left unbound it concretizes to `[Unknown]`, never silently `[]`. Resolution follows the receiver's inferred type, so it also covers nested receivers (`model.service.org.create(..)` — including an intermediate type defined in another module) and a nested field call used as a pipe target (`value |> o.inner.run`). A field declared through a module-local function alias (`run: Action` where `type Action = fn() -> Nil`) is recognized as callable, matching the alias handling already applied to function-typed parameters.

### Fixed

- **A module-level `external effects <module>` declaration now governs a path dependency's inferred module, with its full effect set.** Two problems are fixed. (1) A non-empty module-level external (`external effects dep : [Database]`) was flattened to pure: the declaration carried only the module name, so every `dep.*` call resolved to `[]` and a check could miss the declared effect. The declared set is now preserved. (2) graded source-infers a path dependency that has no spec file of its own, and that inference shadowed the declaration, so a declared module resolved to an inferred `[Unknown]` — and a wrapper module *within the same dependency* that called the declared module was itself polluted to `[Unknown]`. The declaration now governs during the dependency's own inference, so both the module and its in-dependency callers resolve to the declared set. Hex dependencies were already unaffected, since graded never source-infers them. A per-function `external effects mod.fn`, or an authoritative dependency spec or catalog effect, still takes precedence.
