...
- **`Environment` effect + envoy catalog entry.** Reading and writing process environment variables is now a first-class effect (`Environment`) rather than `[Unknown]`. Added a `priv/catalog/envoy@1.0.0.graded` entry mapping `envoy.get`/`set`/`unset`/`all` to `[Environment]`, so any package that reads env vars (config, dotenv, CLI, directory-resolution libraries) infers a precise effect.

### Fixed

- **`@external` (FFI) functions are now `[Unknown]` by default — opt in to a precise effect with an annotation.** Foreign code is opaque: graded cannot see what the native implementation does, so any `@external` function now infers the conservative `[Unknown]` instead of the `[]` an empty (or pure-looking fallback) body would otherwise yield — and that no longer propagates a spurious purity to callers. (Previously graded never inspected the `@external` attribute, so e.g. a JavaScript mutable-cell `set` FFI inferred `[]`.) This holds even when the external carries a Gleam fallback body, since that body only runs on the *other* compile target where the foreign function may still differ. Authors opt in to a precise effect by declaring it with `external effects mod.fn : [...]` (or via the versioned catalog), which wins at resolution time; the inferred `[Unknown]` line for such a function is then dropped so it neither duplicates nor shadows the declaration.
- **Field calls on a record built at several construction sites no longer leak operator bounds.** A function-typed field (the parser-combinator / state-monad idiom `Parser(run: fn(…) -> …)`) gets an inferred effect that is a *union* of operators — one per construction site (`pure` wires `λ_. []`, `do` wires `λin. [Unknown]`). The resolver only treated a single operator as operator-valued, so the union was returned raw and its bounds leaked into the effect set of any first-order function that called the field (`run(p, in) { p.run(in) }` inferred `[fn(_) -> [], fn(in) -> [Unknown]]` instead of `[Unknown]`). The union is now applied to the call's arguments, distributing over the union (`(L ⊔ f ⊔ g)(args) = L ⊔ f(args) ⊔ g(args)`) so operator members beta-reduce and ground members pass through. The result was always sound (it ground-projects to `[Unknown]`), but the leaked bounds were malformed in a first-order signature and not round-trip parseable.
- **`infer` no longer hangs on densely mutually-recursive modules.** Same-module effect resolution re-analysed each callee's body once per distinct call path, so a module with a dense call graph — a recursive-descent parser, a `use`-chained record codec — blew up combinatorially (a 2000-line TOML parser exceeded 90s; some real packages timed out CI entirely). Each function's body analysis is now memoized per module, keyed so the cached result is provably the same as recomputing it. The call graph is partitioned into strongly-connected components (Tarjan's algorithm, `topo.scc_order`): a component of purely first-order functions is *collapsed* — all its members share one full-reachability effect set, computed once, since a public entry's truncated union already equals it — while an effect-polymorphic callee is keyed by name plus its same-component ancestors (the cycle back-edges truncation can cut), where the result genuinely depends on them. Whether a component is first-order is decided deterministically from the syntax — direct `fn(...)` parameters and parameters typed through a module-local function alias (`dec: SizedDecoder(a)` where `type SizedDecoder(a) = fn(...)`) — rather than relying on the best-effort type annotator, whose availability can vary; this keeps the collapse decision (and therefore the result) stable. The same scheme covers the two other transitive-analysis paths — a same-module function lifted as an operator argument, and an inline closure (keyed by its source span, which is what tames a long `use` chain's nested continuations). The memo is threaded through the analysis as an immutable value (a record of dicts) rather than held in mutable process state, keeping resolution referentially transparent. Results are unchanged — only speed; the three corpus packages that previously timed out now infer in 1–5s.
