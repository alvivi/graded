...
- **`graded check` warns on spec lines that match nothing.** A `check` line whose function name matches no function in any project module — usually a missing module qualifier (`check view : []` instead of `check myapp.view : []`) — never runs and passes silently; it's now flagged. A `type` line whose `module.Type.field` matches no field of any project custom type — unqualified or mis-qualified — silently resolves the field to `[Unknown]`; it's now flagged too. Both are reported against the spec file.
