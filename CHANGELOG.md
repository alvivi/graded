...
- **`graded effect <name>` looks up a function or type-field effect without writing anything.** Until now the only way to see an effect was `graded infer`, which rewrites the spec file and the cache; `graded effect myapp/router.handle` prints `effects myapp/router.handle : [Stdout]` and leaves both untouched. `<name>` is either a module-qualified function or a `module.Type.field` type field (`graded effect myapp/repo.Repo.find`), and the output is spec syntax: a higher-order function's parameter bounds are printed with its effect (`effects myapp.apply(f: [Stdout]) : [Stdout]`), and provenance — such as an answer that came from a module-level `external effects` declaration — follows on a `//` comment line, so the whole output parses as `.graded`. Public functions resolve on a fresh checkout with no prior `graded infer`; type fields resolve from declared `type` lines, in whichever form declared them (`type myapp/repo.Repo.find` or the bare `type Repo.find`). A private function, an undeclared field, or an unknown name exits non-zero. A function graded knows about but can't resolve prints `[Unknown]` rather than reporting the name as missing.
...
- **A committed higher-order `effects` line's parameter bounds now apply with that line's own effect term, instead of being replaced by freshly inferred bounds.** A spec line such as `effects app.run(callback: [callback]) : [callback]` contributed only its effect term; the bounds were re-derived from the current source. When spec and source have drifted — the parameter was renamed, or the line was hand-edited — the checker substituted with a mismatched pair (the committed term's variable bound by nothing) and resolved calls into `app.run` wrongly. Term and bounds now come from the same annotation source. A `check` line's bounds stay scoped to that check, and a function declared with `external effects` keeps taking its effect and bounds from the declaration.
