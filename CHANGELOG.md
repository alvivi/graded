...
- **A recursive function passed by name to a higher-order call resolves to its real effect instead of `[Unknown]`.** A self- or mutually-recursive function handed to a higher-order helper by reference (`list.flat_map(children, walk)`) rather than as an inline closure leaked a phantom effect variable that collapsed to `[Unknown]` once type information routed it through the operator-lift path. The recursive reference is already on the analysis stack, so it now contributes nothing — its own effects are captured by the frame analysing it — matching how cyclic local calls are already handled. A pure recursive tree walk like `walk` resolves to `[]`.
