...
- **Calls into girard now resolve from the bundled catalog instead of `[Unknown]`.** A new `girard@2.0.0` catalog entry covers girard's public API, so a consumer's `girard.disk_resolver()` reports `[FileSystem]` and `girard.type_to_string(t)` reports `[]`. The four `annotate*` entry points are polymorphic in the resolver their options carry rather than claiming a fixed `[FileSystem]`: a call whose options graded can't trace reports `[Unknown]`, and a caller-supplied resolver's own effects (`[Stdout]` for one that prints) are kept rather than replaced by the default resolver's.
