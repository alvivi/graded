...
- **Bodyless `@external` (FFI) functions are no longer inferred as pure `[]`.** A function declared with `@external(...)` and no Gleam body is opaque foreign code that can perform any effect, so it now infers the conservative `[Unknown]` instead of the `[]` an empty body would otherwise yield — and that no longer propagates a spurious purity to callers. (Previously graded never inspected the `@external` attribute, so e.g. a JavaScript mutable-cell `set` FFI inferred `[]`.) An `@external` that also has a Gleam fallback body is still analysed from that body; a catalog entry or a user `external effects … : []` line narrows a genuinely-pure FFI back to `[]`.
